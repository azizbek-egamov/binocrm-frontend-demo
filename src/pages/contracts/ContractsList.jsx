...
const ContractsList = () => {
    ...
    const { user } = useAuth();
    const canDeleteContracts = Boolean(user?.can_delete_contracts);
    ...
    const handleDownloadPdf = async (contractId, e) => {
        e?.stopPropagation();
        try {
            toast.loading("PDF yuklab olinmoqda...", { id: `pdf-${contractId}` });

            const response = await contractService.downloadPdf(contractId);
            downloadPdf(response.data, `shartnoma-${contractId}.pdf`);
    ...
    const handleDeleteContract = async (e, id, contractNumber) => {
        ...
        if (!canDeleteContracts) {
            toast.error("Sizda shartnomalarni o'chirish huquqi mavjud emas!");
            return;
        }
